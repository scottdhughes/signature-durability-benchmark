...
All use the "interferon" biological_program assignment. Validation requires
concordant direction for >=8/10 canonical IFN marker genes (STAT1, IRF1, IFIT1,
IFIT2, ISG15, MX1, OAS1, GBP1, CXCL10, RSAD2), matching the paper and frozen
...
MIN_IFN_MARKER_CONCORDANCE = 8


def require_ifn_marker_concordance(expr_df, pheno_df, case_label: str, control_label: str) -> bool:
    """Enforce the expanded-panel IFN admission rule used in the paper."""
    n_ok = verify_markers(expr_df, pheno_df, 'interferon', case_label, control_label)
    if n_ok < MIN_IFN_MARKER_CONCORDANCE:
        print(
            f"  ERROR: {n_ok}/10 IFN admission markers concordant; "
            f"expanded IFN cohorts require >= {MIN_IFN_MARKER_CONCORDANCE}/10"
        )
        return False
    print(f"  Admission check passed: {n_ok}/10 IFN markers concordant")
    return True
...
def process_gse50772():
    ...
    if not require_ifn_marker_concordance(expr_df, pheno_df, 'sle', 'healthy'):
        return False
...
def process_gse13355():
    ...
    if not require_ifn_marker_concordance(expr_df, pheno_df, 'psoriasis', 'healthy'):
        return False
...
def process_gse51808():
    ...
    if not require_ifn_marker_concordance(expr_df, pheno_df, 'dengue', 'healthy'):
        return False
