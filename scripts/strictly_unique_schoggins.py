#!/usr/bin/env python3
"""Test with 41 strictly-unique Schoggins genes (NOT in the Hallmark IFN cores).

This analysis targets the strongest version of the curation-circularity concern:
the expansion cohorts were admitted using a 10-marker IFN check
(STAT1, IRF1, IFIT1, IFIT2, ISG15, MX1, OAS1, GBP1, CXCL10, RSAD2), and reuse
of those same genes could in principle bake the answer into cohort admission.

The 41 strictly-unique Schoggins genes (found in neither Hallmark IFN core)
share ZERO genes with that marker list. If this 41-gene subset still produces
a large within-IFN effect on the expanded k=11 panel, exact admission-marker
reuse is unlikely to explain the result.
"""
...
# The 41 strictly-unique Schoggins genes (NOT in the Hallmark IFN cores)
# None of these are in the marker verification list used during expansion
# cohort admission, so this directly tests the exact-gene-reuse version of
# the curation-level circularity concern.
STRICTLY_UNIQUE = [
...
print("✓ Zero overlap with expansion marker genes — exact marker reuse is ruled out")
...
# Save results
output = {
    "description": "Strictly unique Schoggins (41 genes) test — zero overlap with Hallmark IFN cores or expansion marker genes",
    ...
    "interpretation": (
        "The 41 strictly-unique Schoggins genes share ZERO genes with the 10 IFN "
        ...
        "within-IFN effect, exact admission-marker reuse is unlikely to be the "
