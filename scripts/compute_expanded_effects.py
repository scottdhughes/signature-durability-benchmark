...
overexpression screens. Uses a 76-gene core with 25-36% overlap with MSigDB
Hallmark IFN sets, making it a genuinely orthogonal validation panel.
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# Load signatures (add Schoggins if missing)
# ═══════════════════════════════════════════════════════════════════════════════
...
# Add Schoggins IRG signature if not present
if "schoggins_2011_irg" not in sigs["signature_id"].unique():
    ...
    sigs_expanded = pd.concat([sigs, pd.DataFrame(new_rows)], ignore_index=True)
    sigs_expanded.to_csv(sigs_path, sep="\t", index=False)
    print(f"  Added schoggins_2011_irg signature with {len(new_rows)} genes")
    sigs = sigs_expanded
